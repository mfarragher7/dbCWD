...
#created 2023-01-17
#updated 2023-04-04, working on lake monitoring summit mini project
...
#4105 unique lake-year-month. ~30 * ~6 * ~6 = >5000 makes sense
...
##########################################
# needs work I think. mk function needs higher n than some have. might be fine if it's just summer values....
...
ggplot(yrprosub.droppedgaps, 
       aes(x=year, y=yr.temp.top5m, color=lake)) +
...
hypo.trends = ddply(yrprosub.droppedgaps, 
                          ...
                          first.year = min(year),
                          n.pro = sum(n.profiles),
                          hypo.mean = mean(yr.temp.hypo),
                          hypo.sd = sd(yr.temp.hypo),
                          hypo.min = min(yr.temp.hypo),
                          hypo.max = max(yr.temp.hypo),
                          mk.tau = NA,
                          mk.p = NA,
                          sens.slope = NA,
                          sens.p = NA)
